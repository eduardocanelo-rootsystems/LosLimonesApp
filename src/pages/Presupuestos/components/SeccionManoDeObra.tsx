...
export function SeccionManoDeObra({ items, diasEstimados, catalogo, esAprobado, onChange, onDiasChange }: SeccionManoDeObraProps) {
  ...
  const diasGlobales = parseFloat(diasEstimados) || 0
  ...
  const actualizarDias = (key: string, val: string) => {
    const num = val === '' ? null : (parseInt(val) || null)
    onChange(items.map((i) => i._key === key ? { ...i, dias: num } : i))
  }

  const costoTotal = items.reduce((acc, i) => {
    const d = i.dias ?? diasGlobales
    return acc + i.costo_diario * i.cantidad_empleados * d
  }, 0)
      ...
      <div className="p-6">
        <div className="mb-5 flex items-center gap-4">
          ...
          <p className="mt-5 text-xs text-ink-500">
            Podés sobreescribir los días por fila si un empleado no asiste todos los días.
          </p>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[560px]">
                <thead>
                  <tr className="border-b border-ink-800 bg-ink-900/50">
                    ...
                    <th className="px-4 py-2.5 text-right text-xs font-medium uppercase tracking-wider text-ink-400">
                      Días
                      <span className="ml-1 font-normal normal-case text-ink-600">(vacío = {diasGlobales || '—'})</span>
                    </th>
                ...
                <tbody className="divide-y divide-ink-800">
                  {items.map((item) => {
                    const diasEfectivos = item.dias ?? diasGlobales
                    const sub = item.costo_diario * item.cantidad_empleados * diasEfectivos
                    return (
                      <tr key={item._key} className="hover:bg-ink-900/30">
                        ...
                        <td className="px-4 py-2.5 text-right">
                          <input
                            ...
                            min="1"
                            value={item.dias ?? ''}
                            onChange={(e) => actualizarDias(item._key, e.target.value)}
                            placeholder={diasGlobales > 0 ? String(diasGlobales) : '—'}
                            className="w-20 rounded border border-ink-700 bg-ink-900 px-2 py-1 text-center font-mono text-sm text-ink-100 placeholder-ink-600 focus:border-accent-500 focus:outline-none"
                          />
                        </td>
                        <td className="px-4 py-2.5 text-right font-mono font-medium text-ink-100">
                          {diasEfectivos > 0 ? formatCurrency(sub) : '—'}
                        </td>
